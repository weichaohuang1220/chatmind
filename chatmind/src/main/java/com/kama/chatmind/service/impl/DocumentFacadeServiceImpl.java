...
import com.kama.chatmind.converter.DocumentConverter;
import com.kama.chatmind.exception.BizException;
import com.kama.chatmind.mapper.DocumentMapper;
import com.kama.chatmind.model.dto.DocumentDTO;
...
import com.kama.chatmind.service.DocumentStorageService;
import com.kama.chatmind.service.MarkdownParserService;
...
public class DocumentFacadeServiceImpl implements DocumentFacadeService {
    ...
    public void reprocessDocument(String documentId) {
        ...
        try {
            DocumentDTO documentDTO = documentConverter.toDTO(document);
            String filetype = documentDTO.getFiletype();

            if (!"md".equalsIgnoreCase(filetype) && !"markdown".equalsIgnoreCase(filetype)) {
                throw new BizException("目前仅支持重新处理 Markdown 文档");
            }
            ...
            // 重新处理
            processMarkdownDocument(documentDTO.getKbId(), documentId, documentDTO.getMetadata().getFilePath());
        } catch (JsonProcessingException e) {
            throw new BizException("处理文档时发生序列化错误: " + e.getMessage());
        }
    }
    ...
    private void processMarkdownDocument(String kbId, String documentId, String filePath) {
        try {
            ...
            try (InputStream inputStream = Files.newInputStream(path)) {
                ...
                log.debug("Parsed markdown sections: {}", sections);
                ...
                // 添加 chunk overlap：每个 chunk（除第一个）前面拼接上一个 chunk 的最后 200 个字符
                int overlapSize = 200;
                for (int i = 1; i < sections.size(); i++) {
                    MarkdownParserService.MarkdownSection prevSection = sections.get(i - 1);
                    MarkdownParserService.MarkdownSection currSection = sections.get(i);
                    String prevContent = prevSection.getContent();
                    if (prevContent != null && !prevContent.isEmpty()) {
                        String overlap = prevContent.length() > overlapSize
                                ...
                                : prevContent;
                        String currContent = currSection.getContent() != null ? currSection.getContent() : "";
                        currSection.setContent(overlap + currContent);
                    }
                }
                ...
                for (MarkdownParserService.MarkdownSection section : sections) {
                    ...
                    // 将标题存储到 metadata 中，方便检索时使用
                    String chunkMetadata = "{\"title\":\"" + title.replace("\"", "\\\"") + "\"}";

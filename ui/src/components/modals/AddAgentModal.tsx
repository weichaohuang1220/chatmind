        ...
        <div className="flex-1 h-full relative">
          <div className="px-4 pb-4 overflow-y-scroll">
            ...
            {selectedKey === "skills" && (
              <div>
                <div className="mb-4">
                  <label className="block text-gray-700 font-medium mb-3">技能配置</label>
                  <p className="text-sm text-gray-500 mb-4">
                    选择智能体可以使用的技能组合，每个技能包含多个工具
                  </p>
                  {skills.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      <p>暂无可用技能</p>
                    ...
                    <div className="space-y-3">
                      {skills.map((skill) => {
                        const isSelected = formData.allowedSkills?.includes(skill.id);
                        return (
                          <div
                            key={skill.id}
                            className={`border rounded-lg p-4 cursor-pointer transition-all hover:border-blue-400 hover:bg-blue-50 ${
                            ...
                            onClick={() => {
                              ...
                              if (isSelected) {
                                setFormData({ ...formData, allowedSkills: current.filter(s => s !== skill.id) });
                              } else {
                                setFormData({ ...formData, allowedSkills: [...current, skill.id] });
                              }
                            }}
                          >
                            <div className="flex items-start gap-2">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex flex-wrap gap-1 mt-2">
                                  {skill.tools.map(tool => (
                                    <span key={tool} className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full">{tool}</span>
                                  ))}
                                </div>
                              </div>
                            </div>

...
export async function getSkills(): Promise<GetSkillsResponse> {
  return get<GetSkillsResponse>("/skills");
}

/**
 * 创建技能
 */
export async function createSkill(request: CreateSkillRequest): Promise<CreateSkillResponse> {
  return post<CreateSkillResponse>("/skills", request);
}
...
export async function deleteSkill(skillId: string): Promise<void> {
  return del<void>(`/skills/${skillId}`);
}
...
export async function reprocessDocument(documentId: string): Promise<void> {
  return post<void>(`/documents/${documentId}/reprocess`);
}

/**
 * SSE 确认操作
 */
export async function confirmAction(confirmationId: string, approved: boolean): Promise<void> {
  const response = await fetch(`http://localhost:8080/sse/confirm/${confirmationId}`, {
    method: 'POST',
  ...
  if (data.code !== 200) throw new Error(data.message || 'Confirmation failed');
}
